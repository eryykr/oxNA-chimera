...
bool DRHInteraction::_is_DNA(BaseParticle *p) {
  if (_nucleotide_types[p->index] == 'D') 
      return true;
    else
      return false;
}
...
// bonded (COPYING FROM DNA2 FOR CHIMERIC INTERACTIONS)
number DRHInteraction::_backbone(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(_interaction_type(p,q) == 0) {
		...
		//return (number) 0.f;
		return _backbone_DRH(p, q, false, update_forces);
	}
}
...
void DRHInteraction::check_input_sanity(std::vector<BaseParticle*> &particles) {
	...
	for(int i = 0; i < N; i++) {
		...
		/*
		//making sure there are no bonds between DNA and RNA particles (taking out for chimera ver)
		if(p->n3 != P_VIRTUAL) {
			BaseParticle *q = p->n3;
...
number DRHInteraction::_backbone_DRH(BaseParticle *p, BaseParticle *q, bool compute_r, bool update_forces) {
	if(!DNA2Interaction::_check_bonded_neighbour(&p, &q, compute_r)) {
		return (number) 0.f;
	}
	...
	number rbackr0 = rbackmod - _fene_r0_DRH;

	LR_vector force;
	number energy;

	if(DNA2Interaction::_use_mbf && fabs(rbackr0) > DNA2Interaction::_mbf_xmax) {
		// we use the "log"  potential
		number fene_xmax = -(FENE_EPS / 2.f) * log(1.f - SQR(DNA2Interaction::_mbf_xmax) / FENE_DELTA2);
		number long_xmax = (DNA2Interaction::_mbf_fmax - DNA2Interaction::_mbf_finf) * DNA2Interaction::_mbf_xmax * log(DNA2Interaction::_mbf_xmax) + DNA2Interaction::_mbf_finf * DNA2Interaction::_mbf_xmax;
		energy = (DNA2Interaction::_mbf_fmax - DNA2Interaction::_mbf_finf) * DNA2Interaction::_mbf_xmax * log(fabs(rbackr0)) + DNA2Interaction::_mbf_finf * fabs(rbackr0) - long_xmax + fene_xmax;
		if(update_forces)
			force = rback * (-copysign(1.f, rbackr0) * ((DNA2Interaction::_mbf_fmax - DNA2Interaction::_mbf_finf) * DNA2Interaction::_mbf_xmax / fabs(rbackr0) + DNA2Interaction::_mbf_finf) / rbackmod);
	}
	else {
	
		// we check whether we ended up OUTSIDE of the FENE range
		if(fabs(rbackr0) > FENE_DELTA - DBL_EPSILON) {
			if(update_forces && !DNA2Interaction::_allow_broken_fene) {
